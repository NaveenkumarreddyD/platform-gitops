...
# Declarative: Manage is rendered from the start (no ENABLE_MANAGE toggle, no flag-flip, no
# mid-deploy commit). The Manage apps are auto-generated by account-root once the Suite is Ready;
# this script just nudges + waits for them to converge.
echo ">> account-root will have generated the Manage apps; syncing + waiting for them"
