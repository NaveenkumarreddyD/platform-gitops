...
wait_suite_ready() {
  ...
  while :; do
    ...
    [[ "$elapsed" -ge "$timeout" ]] && {
      ...
      echo ">> the FIRST non-True condition above is the blocker; common causes (see RUNBOOK.md):"
      echo "   SystemDatabaseReady=InvalidConfiguration -> Mongo CA mismatch: RUNBOOK 'MongoDB is not Running' / config checks"
      echo "   SLSIntegrationReady=InvalidConfiguration  -> SLS registration not in Vault: RUNBOOK 'SLS registration is missing'"
      echo "   BASIntegrationReady=NotConfigured         -> DRO registration not in Vault: RUNBOOK 'DRO registration is missing'"
      ...
      echo "   never got created. Fix the operator error first (RUNBOOK 'Certificate failure')."
